  ...
  <div class="app-container">
    ...
    <div class="blog-list">
      <el-table ref="listTable" v-loading="tableDataLoading" :data="listData" tooltip-effect="dark"
        :empty-text="tableDataLoading ? '数据加载中...' : '暂无数据'"
        ...
        <el-table-column label="操作" width="280">
          <template slot-scope="scope">
            ...
            <el-popconfirm style="margin-left: 10px" placement="top-end" confirm-button-text="确定"
              cancel-button-text="取消" icon="el-icon-info" icon-color="red" :title="`你要通过讲师[${scope.row.name}]的审核吗？`"
              @confirm="pass(scope.$index)">
              <el-button slot="reference" type="success" size="mini">通过</el-button>
            </el-popconfirm>
            <el-popconfirm style="margin-left: 10px" placement="top-end" confirm-button-text="确定"
              cancel-button-text="取消" icon="el-icon-info" icon-color="red" :title="`你要永久删除讲师[${scope.row.name}]吗？`"
        ...
        <template #empty>
          <div class="empty-table">
            <img src="@/assets/404_images/404.png" width="120">
            <p>{{ tableDataLoading ? '数据加载中...' : '暂无讲师数据' }}</p>
          </div>
        </template>
      </el-table>
    </div>
    ...
    <!-- 预览简历 -->
    <el-dialog :title="viewResumeDialogTitle" :visible.sync="viewResumeDialogVisible" destroy-on-close width="60vw"
      top="5vh">
      <div style="height: 80vh;overflow: auto">
        ...
        <div v-if="isPdf">
          <pdf v-for="i in pdfPages" :key="i" ref="pdf" :page="i" :src="pdfSrc" />
        </div>
  ...
  methods: {
    ...
    async getList() {
      try {
        ...
        const { data } = await list(this.searchParams)
